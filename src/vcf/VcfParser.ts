...
import { FormatMetadataContainer, parseRecordSample, RecordSample, Genotype } from './SampleDataParser';
import { Metadata, Record } from './Vcf';

export { FormatMetadata, InfoMetadata, NestedInfoMetadata, Value, RecordSample, FormatMetadataContainer, Genotype };
...
function parseLine(line: string, vcf: Container) {
  if (line.charAt(0) === '#') {
    if (line.charAt(1) === '#') {
      ...
      vcf.header = parseHeaderLine(line);
    }
  } else {
